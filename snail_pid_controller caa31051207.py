...
class SNAILParams:
    ...
    """
    # --- Robot geometry ---
    robot_mass:     float = 0.800    # kg
    hose_mass:      float = 0.200    # kg
    wheel_base:     float = 0.120    # m
    pad_radius:     float = 0.040    # m

    # --- VBA mechanism (Table II) ---
    vba_disk_radius:    float = 0.050
    vba_disk_thickness: float = 0.0005
    vba_omega:          float = 2 * math.pi * 175   # rad/s — 175 Hz ERM
...
if __name__ == "__main__":
    ...
    fig.suptitle(f"SNAIL — Raw vs Clean signals  ({inclination_deg}° incline)", fontsize=14, fontweight="bold")
    ...
    fmt(ax, "Trajectory", "x (m)", "y (m)"); ax.legend(fontsize=7)
    ...
    def twin(gs_pos, tr, yr, tc, yc, title, ylabel):
        ...
        fmt(ax, title, "Time (s)", ylabel); ax.legend(fontsize=6.5)
    ...
    fmt(ax, "Velocity tracking", "Time (s)", "v (m/s)"); ax.legend(fontsize=6)
    ...
    fmt(ax, "Yaw rate tracking", "Time (s)", "ω (rad/s)"); ax.legend(fontsize=6)
    ...
    fmt(ax, "Pad tilt θ₁ (pad 1 CCW)", "Time (s)", "θ₁ (°)"); ax.legend(fontsize=6.5)
    ...
    ax.axhline(15, color="grey", ls=":", lw=0.8); ax.axhline(-15, color="grey", ls=":", lw=0.8)
    fmt(ax, "Pad tilt θ₂ (pad 2 CW)", "Time (s)", "θ₂ (°)"); ax.legend(fontsize=6.5)
    ...
    # plt.savefig("/Users/shawnmlee/Library/CloudStorage/OneDrive-SingaporeUniversityofTechnologyandDesign/SUTD/RF Stuff/Writing Papers/ICRA27-SNAIL/Code/snail_simulation_new.png", dpi=150)
